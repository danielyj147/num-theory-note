...
\title{\Huge{Number Theory \& Mathematical Reasoning} \\ Instructor: Dr. Daniel Saracino}
...
\thm{}{Suppose $a$ and $b$ are integers that are not both $0$, so that $(a, b)$ exists.  
...
\thm{}{Every integer $n \geq 2$ is the product of one or more primes.}

\thm{}{There exist infinitely many primes.}

\thm{}{For every integer $n \geq 2$, the factorization of $n$ into primes is unique, 
except for the order in which the factors are written.}

\thm{}{Suppose $m \in \mathbb{Z}^+$ and $a, b \in \mathbb{Z}$.
...
\thm{}{Basic Properties of Congruence Modulo $m$:\\
...
\thm{}{Suppose $m \in \mathbb{Z}^+$, and let $a, b, c, d \in \mathbb{Z}$.\\
...
\mlenma{Division Lemma}{If $a$ and $m$ are integers and $m$ is positive, then there exist integers $q$ and $r$ such that $a = qm + r$
and $0 \le r \le m$}
...
\cor{}{Every integer $n \ge 2$ has a unique representation in the form:
\[
...
\cor{}{Suppose $n \geq 2$ and the prime-power decomposition (ppd) of $n$ is:
\[
n = p_{1}^{e_{1}} \times p_{2}^{e_{2}} \times p_{3}^{e_{3}} \times \cdots \times p_{k}^{e_{k}}.
\]
...
\cor{}{If $n$ is an integer such that $n \geq 2$, then there exists a positive integer $m$ such that $n = m^2$ if and only if all the exponents in the ppd of $n$ are even.}

\cor{Theorem 8-1}{Suppose $a \equiv b \pmod{m}$. Then:
\begin{align*}
a &\equiv b \pmod{m}, \\
...
\cor{Theorem 8-2}{If $p(x)$ is a polynomial with integer coefficients and $a \equiv b \pmod{m}$,  
