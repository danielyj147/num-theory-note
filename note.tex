...
\dfn{Divide}{If $m$ and $n$ are integers, we say that $m$ divides $n$, and we write $m|n$, if there exists some $c \in \mathbb{Z}$ such that $n=mc$.

If $n \neq mc$, $m$ does not divide $n$, and we write $m \nmid n$.
}

\pitf{Zero \textbf{CAN} divide zero}{
  Since $0 = 0 \times c$  is true for any integer $c$, 0 divides 0.
  }
\pitf{Integer $c$ \textbf{CAN} be negative}{
$a \mid b$ and $b \mid a$ does not mean $a = b$.

e.g. a = 4, b = -4
}

\dfn{Greatest Common Divisor (gcd)}{
  If $m$ and $n$ are integers and not both are 0, then the \textit{greatest comon divisor} of $m$ and $n$ is the largest integer that divides both $m$ and $n$. 
  We denote the gcd of $m$ and $n$ by (m, n) or gcd(m, n).  \linebreak
...
\insight{gcd is always positive}{
  e.g., (-45, 27) = 9. (-28, 39) = 1
}


\thm{}{For all integers $a$, $b$, $c$, $d$:
...
\begin{myproof} Since $a \mid b$, there exists $k \in \mathbb{Z}$ such that $b = ak$
...
\thm{}{Suppose $a$ and $b$ are integers that are not both 0, so that $(a, b)$ exists. b\\
...
\dfn{Euclidean Algorithm}{The Euclidean Algorithm is a method for finding the greatest common divisor (GCD) of two numbers.

It works as follows:
\begin{enumerate}
  \item Take two positive integers $a$ and $b$. 
  \item Divide $a$ by $b$. If the remainder is 0, $b$ is the GCD. 
  \item If not, replace $a$ with $b$, and $b$ with the remainder. 
  \item Repeat steps 2-3 until the remainder is 0.
\end{enumerate}
...
\mlenma{Bezout's Lemma}{
  If $m$ and $n$ are integers that are not both zero that $(m, n)$ exists,\\ then 
  there exist $x, y \in \mathbb{Z}$ such that $xm + yn = (m, n)$
} 
